...
const fetcher = async (url) => {
    ...
    if (!res.ok) {
        ...
        error.info = errorData.message || 'Unknown error';
...
const GithubProjects = () => {
    ...
    const { data, error, isLoading, mutate: revalidateData } = useSWR(
        `${GITHUB_API_URL}?sort=updated&per_page=100&page=1`,
        fetcher,
        {
    ...
    const projects = React.useMemo(() => {
        if (!data) return [];
        // Show all public repos, sorted by update date (most recent first)
        const filtered = data
            .filter(project => !project.fork) // Show all non-forked repos
