    ...
    >
        ...
        >
            ...
            >
                ...
                <motion.span
                className={`text-purple-300 font-semibold ${isMobile ? 'hidden' : 'text-base sm:text-lg'}`}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
...
const ContactButton = ({ isMobile = false, onLinkClick }) => (
    <motion.div
        ...
        onClick={onLinkClick}
    >
        <Link href={config.social.linkedinUrl || "/contact"} target={config.social.linkedinUrl ? '_blank' : ''} className={isMobile ? 'w-full' : ''}>
            <Button className={`${isMobile ? 'w-full' : ''} rounded-2xl font-semibold bg-purple-500/10 text-purple-300 hover:bg-purple-500/20 border border-purple-500/30 hover:border-purple-500/50 text-sm sm:text-base px-4 sm:px-6 py-2 sm:py-3 backdrop-blur-sm shadow-lg shadow-purple-500/20`}>
